...
//	private		boolean							prorate					= false;
		...
		)
	{
		...
		try {
			...
			//Military related dues calculation related. - START
			...
			setMilitaryDiscount(membership, dues, militaryDiscountCode, hasMilitaryDiscountRq); 
				
			//Military related dues calculation related. - END
	...
	private void setMilitaryDiscount(Membership membership, MembershipDues dues, String militaryDiscountCode, boolean requestedMilitaryDiscountFlag) throws Exception{
		//if not a military membership and didnot ask for it, do nothing; elseif ask, calculate it. 
		//if a military membership but through grandfather rule, do nothing no matter ask or not; else if not through grandfather rule, doesn't matter whether ask for it or not always calculate it.  
		
		boolean hasMilitaryDiscount = requestedMilitaryDiscountFlag; 
		
		//grand father rule. 
		final String grandMilitaryCd1 = "MILT"; 
		final String grandMilitaryCd2 = "MILTDS";
		
		MembershipAffiliationBP abp = (MembershipAffiliationBP) BPF.get(this.user, MembershipAffiliationBP.class);
		if (abp.isMilitaryMembership(membership)) {						//membership is military already
			String billingCategoryCd = membership.getBillingCategoryCd(); 
			if (billingCategoryCd!=null && (billingCategoryCd.equalsIgnoreCase(grandMilitaryCd1) || billingCategoryCd.equalsIgnoreCase(grandMilitaryCd2))) {
		...
		if (hasMilitaryDiscount) {		//calculate military discount only when necessary
			Collection<Discount> ds = getMilitaryDiscount(militaryDiscountCode, divisionKy);
			Double total = Double.parseDouble(dues.getTotal());
...
//			MembershipDuesCalculatorUtil mcu1 = new MembershipDuesCalculatorUtil(user);
			...
			for(String cov: getUpgradeableCoverages(membership.getCoverageLevelCd(), source)){			
				...
				setMilitaryDiscount(membership, due, "ML", true);
				dues.add(due);			
			}
