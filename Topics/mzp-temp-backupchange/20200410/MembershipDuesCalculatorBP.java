...
//	private		boolean							prorate					= false;
		...
		)
	{
		...
		try {
			...
			//Military related dues calculation  - START
			//TODO
			/*
			 If military membership already , use the checkRequirements(Membership membership, String discountCd) function to determine whether to calculate the discount. 
			 If non military membership and request ML, and in renew, then calculate the membership due with military discount. 
			 */
			...
			if (!abp.isMilitaryMembership(membership) && membership.getPrimaryMember().inRenewal() ) {
				...
				if (checkRequirements(membership, militaryDiscountCode)){
					hasMilitaryDiscountRq = true;	
				}
			}
			//Military related dues calculation  - END
			
			setDiscounts(marketCode); 
			calculateMembershipDuesSC(zipCode, totalFutureNoncancelledAMCount, marketCode, covLevCd, hasMilitaryDiscountRq);
			...
			dues = new MembershipDues(zipCode, associateCount, getCoverageLevelText(), marketCode, 
					getMemberDues(), balance, "0.00", discountItems, donations, true);
			
			
			//Military related dues calculation - START
			if (hasMilitaryDiscountRq) {
				setMilitaryDiscount(dues);	
			}
			//Military related dues calculation - END
			
			//SETTING TOTAL COST -- START 
			//	assuming the totalcost was returned from Line 171 of this version in the class properties colMdues
			// 	colMdues is a collection of MemberDues, each member dues has a collection of 
			BigDecimal totalCost = BigDecimal.ZERO;
			if (colMdues != null) {
				for (MemberDues md: colMdues) {
					if (md.getDuesCostItems() != null) {
						for(DuesCostItem di: md.getDuesCostItems()) {
							if (di.getTotalCost()!=null) {
								totalCost = totalCost.add(di.getTotalCost());
							}
						}
					}
				}
			}
			dues.setTotalCost(totalCost.toString());
	...
	public void setMilitaryDiscount(MembershipDues dues) throws Exception{
		BigDecimal militaryDiscountTotal = BigDecimal.ZERO;
		BigDecimal totalCost = BigDecimal.ZERO;
		
		for (MemberDues d: dues.getMemberDues()) {
			for(DuesCostItem dci: d.getDuesCostItems()) {
				if (dci.getMilitaryDiscount().compareTo(BigDecimal.ZERO) != 0 ) {
					militaryDiscountTotal = militaryDiscountTotal.add(dci.getMilitaryDiscount()); 
				}
				
				if (dci.getTotalCost().compareTo(BigDecimal.ZERO) != 0 ) {
					totalCost = totalCost.add(dci.getTotalCost());
				}
				
			}
		}
		
		dues.setMilitaryDiscount(militaryDiscountTotal.toString());
		dues.setTotalCost(totalCost.toString());
	}
	
	public BigDecimal getMilitaryDiscountDuesCost(BigDecimal dues, String militaryDiscountCode) throws Exception{
		
			Collection<Discount> ds = getMilitaryDiscount(militaryDiscountCode, divisionKy);
			Double total = dues.doubleValue();
			BigDecimal militaryDiscountTotal = BigDecimal.ZERO; 
			
			if (ds!=null) {
				for (Discount d: ds) {
					
					boolean isPercent = d.isPercent(); 
					if (!isPercent) {
						//BUG here. 
						militaryDiscountTotal = d.getAmount();
					} else {
						//comes here right now. 
						BigDecimal t = BigDecimal.valueOf(total);
						militaryDiscountTotal = t.multiply(d.getAmount()).divide(new BigDecimal("100"));
						
						militaryDiscountTotal = militaryDiscountTotal.setScale(2, RoundingMode.HALF_UP);
				...
				String marketCode, String source) 
	{
		...
		try {
			...
			//associate count
			validateAssociateCount(newAssociateCount);
			...
			if (membership.getPrimaryMember().inRenewal() ) {
				String militaryDiscountCode = "ML";
			...
			for(String cov: getUpgradeableCoverages(membership.getCoverageLevelCd(), source)){			
				...
				calculateMembershipDuesSC(zipCode, totalFutureNoncancelledAMCount, marketCode, cov, hasMilitaryDiscountRq);
				MembershipDues due =new MembershipDues(zipCode, newAssociateCount, covText, marketCode, colMdues, "0.00", 
						membership.getPaymentAt().toString(), discountItems, donations, false);
				
				if (hasMilitaryDiscountRq) {
					setMilitaryDiscount(due);
				}
    ...
    private boolean validateAndInitializeCalculateDues(String zipCode, int addNewAssociateCount, String coverageLevel, 
    	...
    	try {
    		result = validateZipCode(zipCode, membershipNumber);
    ...
    private boolean validateZipCode(String zipCode, String membershipID) throws Exception{
		//zipCode
		if(zipCode == null){
	...
	public void calculateMembershipDuesSC(String zip, int totalFutureNoncancelledAMCount, String marketCode, String coverageLeverCd, boolean hasMilitaryDiscountRq)
	{
		...
		try {
			...
			//To fix the bug created when non-transfer membership'zip code is not in terriority.
			divisionKy = membership.getDivisionKy(); 
			branchKy = membership.getBranchKy();
			state = membership.getDuesState();
			...
			debug(finalCdArray, "NEW~m", false);
			...
			debug(finalCdArray, "NEW~s", false);
			...
			debug(finalCdArray, "NEW~r", false);
				
			covLvlDescr = membershipUtilBP.getCoverageByMZPType(covLevCdUpperCase);		
			colMdues = createMembershipDues(finalCdArray, totalFutureNoncancelledAMCount, coverageLevels, isExisting, membership, false, hasMilitaryDiscountRq);
	 ...
	 * @return colMdues
	 */
	private  Collection<MemberDues> createMembershipDues(CostData[] cdArray, int assCnt, SortedSet<CoverageLevel> coverageLevels, boolean isExistingMembership, Membership ms, boolean isRenewal, boolean hasMilitaryDiscountRq) {
					...
					if (cd.getAssociateId().equals(String.valueOf(i))){
						...
						if(isExistingMembership)
						{
							if (cd.isDowngrading()){
								...
								DuesCostItem dci = null;
								if(discounts != null && discounts.size() > 0){
									dci = new DuesCostItem(cd.getRiderCompCd(), covLvlDescription, calculateCostDataWithDiscount(cd, i, false, cd.getProratedCost(), ms, isRenewal), new BigDecimal("0.00")); 
								} else {								
									dci = new DuesCostItem(cd.getRiderCompCd(), covLvlDescription, cd.getProratedCost(), new BigDecimal("0.00"));
								}
								
								if (cd.isUpgrading()) {
									BigDecimal costItemDues = new BigDecimal (dci.getDues()); 
									dci.setTotalCost(costItemDues);
									
									//military discount handling if needed  
									if (hasMilitaryDiscountRq) {
										BigDecimal costItemDuesWithMLDiscount = getMilitaryDiscountDuesCost(costItemDues, "ML"); 
										dci.setMilitaryDiscount(costItemDuesWithMLDiscount);
									}	
								} else {
									//existing paying component. 
									//since the advance pay, set all military discount to the basic rider of primary member 
									if (cd.getRiderCompCd().equalsIgnoreCase("BS") && cd.getMemberType().equalsIgnoreCase("P")){
										MembershipServiceBP serviceBp = (MembershipServiceBP) BPF.get(User.getGenericUser(), MembershipServiceBP.class);
										
										BigDecimal costItemDues  = serviceBp.getMembershipInRenewalBillTotalCost(user, ms, false);
										dci.setTotalCost(costItemDues);
	...
	public Collection<Discount> getMilitaryDiscount(String discountcd, BigDecimal divisionKy) throws Exception{
		if (discountcd == null || discountcd.trim().equals("")) {
			return null;
		}
		
		ArrayList discountCds = new ArrayList();
		discountCds.add(discountcd); 
       
		ArrayList<SearchCondition> condsDiscountCd = new ArrayList<SearchCondition>();
		condsDiscountCd.add(new SearchCondition(Discount.DISCOUNT_CD, SearchCondition.EQ, discountCds));
				
		return Discount.getDiscountList(user, condsDiscountCd, divisionKy, null, null);
		
	}	
	
	private boolean checkRequirements(Membership membership, String discountCd) throws Exception {
		boolean retValue = false;
		
		BigDecimal divisionKy = membership.getDivisionKy();
		DiscountBP dbp = BPF.get(user, DiscountBP.class); 
		
		Collection<Discount> ds = getMilitaryDiscount(discountCd, divisionKy);
		if (ds!=null) {
			for (Discount discount: ds) {
				retValue = dbp.checkRequirements(discount, membership); 
