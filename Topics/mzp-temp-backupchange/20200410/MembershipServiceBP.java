...
//			}
	...
	public BaseMembershipDues CalculateBaseDuesEnroll(String zipCode, int associateCount, String marketCode, String[] cls)
	{
		BaseMembershipDues baseDues = null;
		try {
			
			if (getLogger().isDebugEnabled()){			
				getLogger().debug("**************************\n Calculate Base Enroll Dues");
				getLogger().debug("Zip Code: " + zipCode);
				getLogger().debug("Associate Count: " + associateCount);
				getLogger().debug("Market Code: " + marketCode);
				getLogger().debug("**************************\n");			
			}
			String DEBUG_MODULE = "WM_CalculateBaseDuesEnroll_" + Calendar.getInstance().getTimeInMillis() +"";
			MembershipUtilBP mubp = MembershipUtilBP.getInstance();
			...
			for(CoverageLevel cl : coverages){		
				...
				if (flag) {
					//Prakash - 07/20/2018 - Dues By State - Start
					mcu.CalculateMembershipDuesEnroll(zipCode, associateCount, cl.getCoverageLevelCd(), marketCode, billingCategoryCd, branchKy, divisionKy, regionCd, state, coverages);
					//Prakash - 07/20/2018 - Dues By State - End
					dues.add(new MembershipDues(zipCode, associateCount, mcu.getCoverageLevelText(), marketCode, mcu.getMemberDues(), null, null, null, null, true));	
				}
			}
			...
			if(enrollReq !=null){			
				...
				try
				{
					...
					if (isGiftFreeMembershipEnroll){
						...
						/*if (isEnrollByEcheck(enrollReq)) {
							ms = enrollBp.processEnrollByEcheck(enrollReq, true,paymentResponse);
						} else {
							ms = enrollBp.processEnroll(enrollReq, true,paymentResponse);
						}
						*/
	...
	private boolean isEnrollByEcheck(MembershipEnrollRequest enrollReq) {
		if (enrollReq.getPaymentParams() !=null && enrollReq.getPaymentParams().getCheck()!=null){
			return true;
		}
...
//				sa = new SalesAgent(user, req.getSalesAgentID().toUpperCase());
			...
			//SETTING TOTAL COST -- START 
			
			//need to get the bill total cost when advance pay happen, 
			//BigDecimal duesTotal = new BigDecimal(dues.getTotal());
			BigDecimal totalCost = getMembershipInRenewalBillTotalCost(user, membership, false);
			
			/*
			//handle donation.
			BigDecimal dAmount = BigDecimal.ZERO; 
			Collection<Donation> cDonations = membershipCompDues.getDonationItem();
			
			if (cDonations !=null) {
				for (Donation d: cDonations) {
					dAmount = d.getAmountDue(); 
					break; 
				}
			}
			
			totalCost = totalCost.subtract(dAmount);
			*/
			membershipCompDues.setTotalCost(totalCost.toString());
			
			//SETTING TOTAL COST -- END
...
//										
				...
				//TODO - First Responder code handling place holder. 
	...
	private void enrollingMembershipMilitary (Membership membership, MembershipUpdateMemberRequest request) throws Exception {
		//TODO - military membership flag
		MembershipAffiliationBP abp = (MembershipAffiliationBP) BPF.get(this.user, MembershipAffiliationBP.class);
		if (!abp.isMilitaryMembership(membership)) {
			if (request.getSimpleMembership().getMilitaryFlag()!=null && request.getSimpleMembership().getMilitaryFlag()) {
				abp.insertMmembershipCd(membership, "MS");
				membership.addComment("Military status enrolled");
				...
				membership.save();	
				BPF.get(user,PaymentPosterBP.class).postZeroDollarPayment(membership, "ZP-MakePayment", "MM", 
						((MemberzPlusUser)user).getBranchKy(), "N", "P", null);
/*				membership.save();	
*/				
				/*
...
//					}
	...
	public MembershipDues GetMembershipBalanceWithMarketCode(String zipCode, String marketCode, String membershipNumber, String salesAgentID, String[] requestDiscounts) {
		MembershipDues dues = null;		
		try {
			...
			if(membershipNumber != null && !membershipNumber.equals("")) // existing membership
			{
				...
				//need to get the bill total cost when advance pay happen, 
				//BigDecimal duesTotal = new BigDecimal(dues.getTotal());
				BigDecimal duesTotal = getMembershipInRenewalBillTotalCost(user, membership, true);
				
				//handle donation.
				BigDecimal dAmount = BigDecimal.ZERO; 
				Collection<Donation> cDonations = dues.getDonationItem();
				...
				duesTotal = duesTotal.subtract(dAmount);
				dues.setTotalCost(duesTotal.toString());
				
				if (requestDiscounts!=null) {
					String militaryDiscountCode = "ML";
					boolean hasMilitaryDiscountRq = false;

					MembershipAffiliationBP abp = (MembershipAffiliationBP) BPF.get(this.user, MembershipAffiliationBP.class);
					if (!abp.isMilitaryMembership(membership) && membership.getPrimaryMember().inRenewal() ) {
						for (String dc: requestDiscounts){
							if (dc.equalsIgnoreCase(militaryDiscountCode)) {
								hasMilitaryDiscountRq = true;
								break;
							}
						}
					}
					
					if (hasMilitaryDiscountRq) {
						MembershipDuesCalculatorBP dcbp = (MembershipDuesCalculatorBP) BPF.get(User.getGenericUser(), MembershipDuesCalculatorBP.class);
						
						BigDecimal militaryDiscount = dcbp.getMilitaryDiscountDuesCost(duesTotal, militaryDiscountCode);
						dues.setMilitaryDiscount(militaryDiscount.toString());
					}
				}
			}
	...
	//this method was extracted from PaymentBean and payment.jsp to get the Total Cost value of payment screen.
	public BigDecimal getMembershipInRenewalBillTotalCost (User user, Membership membership, boolean includeDonation) throws Exception {
		SimpleEditor simpleEditor = new SimpleEditor(user);
		SimpleVO _paymentVO = null;
		SimpleVO _feeVO = null;
		
		 BigDecimal totalCost = BigDecimal.ZERO;
		
		 if (membership != null) {
			 
             java.util.ArrayList criteria = new java.util.ArrayList();
             criteria.add(new SearchCondition("MEMBERSHIP_KY", SearchCondition.EQ, membership.getMembershipKy()));
             
             ArrayList<String> feeSt = new ArrayList<String> ();
             feeSt.add("A");
             feeSt.add("P");
             criteria.add(new SearchCondition("STATUS", SearchCondition.IN, feeSt));

             _feeVO = simpleEditor.findByCriteria("MZ_MEMBERSHIP_FEES", criteria);
             _feeVO.first();
             
             BigDecimal _totalOtherFees = new BigDecimal(0.00).setScale(2);
             BigDecimal _totalEnrollmentFees = new BigDecimal(0.00).setScale(2);
             BigDecimal _totalDonations = getTotalDonations(false, membership).setScale(2);
             BigDecimal _paidDonations = getTotalDonations(true, membership).setScale(2);
             
             boolean hasFees = true;
             _feeVO.beforeFirst();
             while(_feeVO.next()) {
               hasFees = true;
                 if(_feeVO.getBigDecimal("FEE_AT") != null && _feeVO.getStringNvl("WAIVED_FL","N").equals("N")) {
                     if(_feeVO.getSafeString("FEE_TYPE").equals("ENRL")) {
                         _totalEnrollmentFees = _totalEnrollmentFees.add(_feeVO.getBigDecimal("FEE_AT").setScale(2));
                     }
                     else {
                         _totalOtherFees = _totalOtherFees.add(_feeVO.getBigDecimal("FEE_AT").setScale(2));
                     }
                 }
             }
             _feeVO.first();
             ...
             //this method was extracted from PaymentBean and payment.jsp to get the Total Cost value of payment screen.
			 _paymentVO = getPaymentVO(membership); 
			 
             BigDecimal _duesCost=_paymentVO.getBigDecimal("DUES_COST_AT").setScale(2);
             BigDecimal _duesAdj=_paymentVO.getBigDecimal("DUES_ADJUSTMENT_AT").setScale(2);
             BigDecimal _totalDues=_duesCost.add(_duesAdj);
             BigDecimal _discountAt = _paymentVO.getBigDecimal("DISCOUNT_AT").setScale(2);

             totalCost = _totalDues.add(_totalEnrollmentFees).add(_totalOtherFees).add(_discountAt);
             if (includeDonation) {
            	 totalCost = totalCost.add(_totalDonations); 
             }
	...
	//this method was extracted from PaymentBean and payment.jsp to get the Total Cost value of payment screen.
	private SimpleVO getPaymentVO(Membership membership) throws Exception {
		
		SimpleVO paymentVO = null;
		Connection conn = null;
		try {
			
			if (membership!=null) {
				java.util.Date costEffectiveDt = membership.getPrimaryMember().getBasicRider().getCostEffectiveDt();
				String sCostEffectiveDt = new SimpleDateFormat("dd-MMM-YYYY").format(costEffectiveDt);
				StringBuffer sb = new StringBuffer(); 
				sb.append("SELECT a.membership_ky, ");
				sb.append("  a.membership_id,");
				sb.append(" decode(a.status,'C',0,nvl(sum(r.dues_cost_at),0)) as dues_cost_at, ");
				sb.append(" decode(a.status,'C',0,nvl(sum(r.dues_adjustment_at),0)) as dues_adjustment_at, "); 
				sb.append("  mz_membership_util.get_discount_at(a.membership_ky) as discount_at ");
				sb.append("FROM mz_membership a, ");
				sb.append("mz_rider      r ");
				sb.append("WHERE a.membership_ky = r.membership_ky "); 
				sb.append(" and  a.membership_ky = ? ");
				sb.append(" and ((r.status in ('P', 'A') or a.status = 'C') ");
				sb.append(" 		or (r.status = 'C' and r.cost_effective_dt >= ? ");
				sb.append(" 					)");
				sb.append(" ) ");
				sb.append(" group by a.membership_ky,");
				sb.append(" 		 a.membership_id, ");
				sb.append(" 		 a.unapplied_at, ");
				sb.append("       	 a.advance_pay_at,");
				sb.append(" 		 a.status");
//				System.out.println(sb.toString());
				
				String sql = sb.toString();
				paymentVO = new SimpleVO();
				
				conn = ConnectionPool.getConnection();
				
				paymentVO.setCommand(sql);

				paymentVO.setBigDecimal(1,membership.getMembershipKy());
				paymentVO.setString (2,sCostEffectiveDt);
				
				paymentVO.execute(conn);
	 ...
	 private BigDecimal getTotalDonations(boolean paidFl, Membership membership) throws SQLException, ObjectNotFoundException{
			BigDecimal totalDonations = BigDecimal.ZERO;
			ArrayList<SearchCondition> criteria = new ArrayList<SearchCondition>();
			criteria.add(new SearchCondition(DonationHistory.MEMBERSHIP_KY, SearchCondition.EQ, membership.getMembershipKy()));
			ArrayList<String> statuses = new ArrayList<String>();
			statuses.add("A");
			statuses.add("P");
			criteria.add(new SearchCondition(DonationHistory.STATUS, SearchCondition.IN, statuses));
			criteria.add(new SearchCondition(DonationHistory.COST_EFFECTIVE_DT, SearchCondition.GT + SearchCondition.EQ, 
					membership.getPrimaryMember().getBasicRider().getCostEffectiveDt()));
			
			
	    	for (DonationHistory d : membership.getDonationHistoryList(criteria, null)){
	    		if (paidFl)
	    			totalDonations = totalDonations.add(d.getPaymentAt());
	    		else
	...
	public BaseMembershipDuesShort transformBaseDues(BaseMembershipDues baseDues) {
		BaseMembershipDuesShort baseDuesShort = null; 

		Collection <MembershipDuesShort> alMembershipDuesShort = new ArrayList<MembershipDuesShort>(); 
		
		for (MembershipDues msd: baseDues.getBaseMemberhipDues()) {
			MembershipDuesShort msdShort = new MembershipDuesShort();
			msdShort.setCoverageLevel(msd.getCoverageLevel()); 
			
			Collection <MemberDuesShort> memberDuesShort = new ArrayList<MemberDuesShort>  ();
			
			for (MemberDues md: msd.getMemberDues()) {
				MemberDuesShort mdShort = new MemberDuesShort();
				
				mdShort.setAssociateId( md.getAssociateId());
				mdShort.setTotal(md.getTotal()); 

				Double mTotalFee = Double.parseDouble("0");
				
				for (DuesCostItem dci: md.getDuesCostItems()) {
					if (dci.getRiderType().equalsIgnoreCase("MC") || dci.getRiderType().equalsIgnoreCase("RV")) {
						mdShort.setRvRiderTypeCode(dci.getRiderType()) ;
						mdShort.setRvTotal(dci.getTotal());
					}
					mTotalFee += Double.parseDouble(dci.getFees());
				}
				
				mdShort.setFee(mTotalFee.toString());
				
				memberDuesShort.add(mdShort);
			}
			
			msdShort.setMemberDues(memberDuesShort);
			
			alMembershipDuesShort.add(msdShort); 
			
		}
		
		baseDuesShort = new BaseMembershipDuesShort(alMembershipDuesShort); 
		
		return baseDuesShort;
	}
