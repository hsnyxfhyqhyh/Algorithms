...
//				}
			...
			else // new membership
			{
				...
				if(discounts != null && discounts.length > 0){
					...
					if (requestedDisct.contains("ML")) {
						requestedDisct.remove(militaryDiscountCode); 
		...
		try {
			...
			for(CoverageLevel cl : coverages){		
				...
				if (flag) {
					...
					//Prakash - 07/20/2018 - Dues By State - End
					Membership ms = mcu.getMembership(); 
					MembershipDues due = new MembershipDues(zipCode, associateCount, mcu.getCoverageLevelText(), marketCode, mcu.getMemberDues(), null, null, null, null, true) ;
					
					MembershipDuesCalculatorBP bp = (MembershipDuesCalculatorBP) BPF.get(User.getGenericUser(), MembershipDuesCalculatorBP.class);
					bp.setMilitaryDiscount(ms,  due, "ML", true);
			...
			if(enrollReq !=null){			
				...
				try
				{
					...
					if(ms.getMembershipId() !=null)
					{
						...
						//Military Discount handling - START
						DiscountBP dbp = BPF.get(user, DiscountBP.class); 
						dbp.applyDiscounts(ms);	
						ms.save();	
						//Military Discount handling - END						
